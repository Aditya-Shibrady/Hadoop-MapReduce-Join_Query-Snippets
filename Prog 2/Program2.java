...
//putting average and the corresponding business ids in the tree map
review_id.put(avg, bId_List); 
}
}

@Override
protected void cleanup(Reducer<Text, FloatWritable, Text, NullWritable>.Context context)throws IOException, InterruptedException 
{
...
{
List<Text> businessidTempList = review_id.get(businessidIterator.next());
for(Text t : businessidTempList)
{
if(count>=10) 
break;
context.write(new Text(t.toString()), NullWritable.get());
count++;
}
}
...
Job job = new Job(config, "Program2"); // creates a job with name "Program2"
...
FileInputFormat.addInputPath(job, new Path(otherArgs[0])); // sets the HDFS path of the input data
FileOutputFormat.setOutputPath(job, new Path(otherArgs[1])); // sets the HDFS path for the output
System.exit(job.waitForCompletion(true) ? 0 : 1); // Waits till job completion
}
}
